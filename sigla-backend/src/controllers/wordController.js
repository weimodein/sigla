    ...
    if (hasLandmarkData) {
      ...
      if (isMotionBatch) {
        ...
        const records = await Promise.all(sequence.map(async (seq, idx) => {
          ...
          // Upload all 30 frames so the admin can review the full motion sequence
          const imageUrls = await Promise.all(frameImages.map((base64, i) => saveImage(base64, idx * 30 + i)));
